...
// CORS flexible + Accept: application/json + logs de diagnóstico.
...
export default async function handler(req, res) {
  try {
    ...
    if (!isAllowed(origin)) return res.status(403).json({ error: "Origin not allowed" });
    ...
    // Fallback: si NX_GATE no está (p.ej. en Preview), usa el host actual
    ...
    // Logs NO sensibles
    console.error("[Prepare][debug] ResponseUrl:", ResponseUrl, "amount:", amountCents, "store(len):", String(STORE||"").length);
    ...
    return res.status(200).json({ url: payUrl, clientTransactionId });
